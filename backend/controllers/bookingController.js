...
const generateBookingReference = () => {
  return `BK${Date.now().toString().slice(-8)}${Math.random()
    .toString(36)
    .substr(2, 4)
    .toUpperCase()}`;
};
...
export const createRouteBasedBooking = async (req, res) => {
  try {
    ...
    const nextVehicleInfo = await getNextVehicleInQueueHelper(
      existing_route_id
    );
    ...
    if (!nextVehicleInfo) {
      return res.status(404).json({
        success: false,
        message:
          "No vehicle available at position 1 for this route. Please try again later.",
      });
    }
  ...
  try {
    ...
    if (!owner_id || !vehicle_id || !scheduled_pickup || !total_amount_needed) {
      return res.status(400).json({
        success: false,
        message:
          "Missing required fields: owner_id, vehicle_id, scheduled_pickup, total_amount_needed",
      });
    }
    ...
    if (
      !route_points ||
      !Array.isArray(route_points) ||
      route_points.length < 2
    ) {
      return res.status(400).json({
        success: false,
        message:
          "route_points array with at least 2 points (pickup and dropoff) is required for custom bookings",
      });
    }
        ...
        for (let i = 0; i < route_points.length; i++) {
          ...
          await pool.execute(
            ...
            [
              ...
              point.address || "",
              i + 1,
              point.expected_time || scheduled_pickup,
            ]
          );
        }
      }
  ...
  try {
    ...
    for (let booking of allBookings) {
      ...
      booking.payment_amount =
        paymentAmount !== null
          ? paymentAmount
          : parseFloat(booking.total_amount_paid || 0);
...
export const getBookingDetails = async (req, res) => {
  try {
    ...
    const isOwner =
      userOwnerProfileId !== null && booking.owner_id === userOwnerProfileId;
    const isDriver =
      userDriverProfileId !== null && booking.driver_id === userDriverProfileId;
    ...
    if (shouldShowPassenger && userPassengerRecord.length > 0) {
      ...
      if (passengerRecordId) {
        userPassenger = transformedPassengers.find(
          (p) => p.ID == passengerRecordId && p.linked_user_id === userId
        );
      } else {
        userPassenger = transformedPassengers.find(
          (p) => p.linked_user_id === userId
        );
      }
    }
    ...
    if (shouldShowParcel && userParcelRecord.length > 0) {
      ...
      if (parcelRecordId) {
        userParcel = transformedParcels.find(
          (p) => p.ID == parcelRecordId && p.user_id === userId
        );
      } else {
...
export const cancelBooking = async (req, res) => {
  try {
    ...
    await pool.execute(
      ...
      [bookingId, userId, cancellationType, cancellation_reason || null]
    );
...
export const addPassenger = async (req, res) => {
  try {
    ...
    const pickupAddress =
      ...
      (typeof pickup_point === "string" ? pickup_point : null) ||
      null;
    const dropoffAddress =
      dropoff_address ||
      dropoff_point?.address ||
      (typeof dropoff_point === "string" ? dropoff_point : null) ||
      null;
    ...
    const [result] = await pool.execute(
      ...
      [
        ...
        id_number || null,
        code,
        JSON.stringify(pickup_point || null),
        JSON.stringify(dropoff_point || null),
        pickupAddress,
        dropoffAddress,
        is_primary,
        next_of_kin_first_name,
...
export const addParcel = async (req, res) => {
  try {
    ...
    const receiver_code = Math.random()
      .toString(36)
      .substr(2, 10)
      .toUpperCase();
...
export const updateBookingStatusDriver = async (req, res) => {
  try {
    ...
    // Update status
    await pool.execute("UPDATE bookings SET booking_status = ? WHERE id = ?", [
      booking_status,
      bookingId,
    ]);
...
export const listOfUpcomingTrips = async (req, res) => {
  try {
    ...
    console.log("Fetching upcoming trips for user ID:", userId);
    // Get user's driver profile ID
    // Get user's driver profile ID
    const [driverProfiles] = await pool.execute(
      "SELECT ID FROM driver_profiles WHERE user_id = ?",
...
`,
    ...
    //return the list divided as upcomingtrips and alltrips
  } catch (error) {
...
export const assignDriver = async (req, res) => {
  try {
    ...
    // Update booking
    await pool.execute("UPDATE bookings SET driver_id = ? WHERE id = ?", [
      driver_id,
      bookingId,
    ]);
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Error moving vehicle to end of queue after booking:",
          queueError
        );
        queueResult = {
          success: false,
      ...
      res.status(201).json({
        success: true,
        message:
          "Booking executed successfully. Vehicle moved to end of queue.",
        booking: {
          id: bookingId,
          booking_reference,
          booking_status: "pending",
...
export const getExistingRouteDetails = async (req, res) => {
  ...
  try {
    ...
    console.error(
      "Error fetching existing route details for booking ID:",
      bookingID,
      error
    );
